...
class Orator {
  ...
  async _parseParamsForCommand (params, msg, bot) {
    ...
    if (!cmd) {
      const dbGuild = msg.channel.guild
        ...
        : undefined
      const prefix = (dbGuild && dbGuild.prefix)? dbGuild.prefix:this.defaultPrefix
  ...
  async _tryToExecute (bot, context) {
    ...
    if (params.length < command.parameters.length) {
      ...
      let precom = context.msg.content.split(command.name);
      let a = precom.shift() + command.name;

      return this._badCommand(context, 'insufficient parameters! Usage:  `'+a.toLowerCase()+' <'+command.parameters.join("`> <`")+">`")
    }
    ...
    try {
      ...
      response =
        'There was an error processing your request, please try again later. Error Message ```'+error+'```';
    }
