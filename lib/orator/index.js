...
class Orator {
  ...
  createDirectMessage (me, msg, content, file, notify = true) {
    return msg.author.getDMChannel()
      ...
      .then(async (success) => 'Please check your DMs, Thank you!')
      .catch(async (error) => {
   ...
   */
  async processMessage (bot, msg, useDefault) {
  ...
  async _parseParamsForCommand (params, msg, bot, useDefault) {
    ...
    if (!cmd) {
      const dbGuild = msg.channel.guild
        ? await bot.SQLHandler.getGuild(msg.guildID)
        : undefined
      const prefix = (!useDefault && (dbGuild && dbGuild.prefix)) ? dbGuild.prefix
        : this.defaultPrefix
  ...
  async _tryToExecute (bot, context) {
    ...
    if (params.length < command.parameters.length) {
      const precom = context.msg.content.split(command.name)
      const a = precom.shift() + command.name

      return this._badCommand(context, `insufficient parameters! Usage:  \`${a
        .toLowerCase()} <${command.parameters.join('> <')}>\``)
    }
    ...
    try {
      ...
      bot.CommandStats.addData(command.name, context.msg.author.id, context.msg.guildID, context.msg.content)
      // console.log(!context.msg.member, !command.permissionNode, !!bot.PermissionsHandler, !bot.PermissionsHandler.checkForPerm(context.msg.member, command.permissionNode))
       context.msg.member || !command.permissionNode || (bot.permissionsHandler && !bot.permissionsHandler.checkForPerm(context.msg.member, command.permissionNode))
      if (!context.msg.member || !command.permissionNode || (bot.permissionsHandler && !bot.permissionsHandler.checkForPerm(context.msg.member, command.permissionNode))) {
        response = await command.run(bot, context)
      } else {
        response = `I cannot process your command because you lack the permission \`${command.permissionNode}\``
      }
    } catch (error) {
      ...
      response =
        ...
        please try again later. Error Message \`\`\`${error}\`\`\` I suggest reporting the error with \`\`\`daz report yes\`\`\` to my bot developer!`
    }
  ...
  async hasPermission (bot, context) {
    ...
    if (command.dmOnly) {
      if (msg.channel.guild) {
        return {
          ok: false,
          message: 'Apologies, this command is restricted to DMs only!'
        }
      }
    } else if (command.guildOnly && !msg.channel.guild) {
      return {
        ok: false,
        message: 'Apologies, this command is restricted to Servers only!'
      }
    }
  ...
  _sendHelp (prefix, msg, me) {
    return this.replyToMessage(
      me,
      msg,
      `Hello! The prefix in this server is \`${prefix}\`, try \`${prefix} help\``
    )
  }
}
