...
class Orator {
  ...
  async _parseParamsForCommand (params, msg, bot, useDefault) {
    ...
    if (!cmd) {
      const dbGuild = msg.channel.guild && bot.SQLHandler
        ? await bot.SQLHandler.getGuild(msg.guildID)
        : undefined
	console.log(dbGuild,"Eris-Boiler");
  ...
  async _tryToExecute (bot, context) {
    ...
    try {
      ...
      if (!context.msg.member || !command.permissionNode || (bot.permissionsHandler && await bot.permissionsHandler.checkForPerm(context.msg.member, command.permissionNode))) {
