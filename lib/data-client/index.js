...
class DataClient extends Client {
  ...
  async setupCommands () {
    ...
    for (let i = 0; i < iter.length; i++) {
      ...
      const reqParams = cmd.parameters.map((x, ind) => {
        return {
          name: `r${ind}`,
          description: x || '...',
          type: 3,
      ...
      const optionalParams = cmd.optionalParameters.map((x, ind) => {
        return {
          name: `o${ind}`,
          description: x || '...',
          type: 3,
      ...
      const subCommands = await Promise.all(Array.from(cmd.subCommands.keys())
        .map(async (key, index) => {
          ...
          const scmdreqParams = scmd.parameters.map((x, inn) => {
            return {
              name: `r${inn}`,
              description: x || '...',
              type: 3,
          ...
          const scmdoptionalParams = scmd.optionalParameters.map((x, inn) => {
            return {
              name: `o${inn}`,
              description: x || '...',
              type: 3,
          ...
          const dataobj = {
            ...
            type: 1,
            options: scmdreqParams.concat(scmdoptionalParams)
      ...
      const datapost = {
        ...
        options: subCommands.length ? subCommands : (reqParams).concat(optionalParams)
      }
      if (!datapost.options.length) {
        delete datapost.options
      }
      await axios.post(`https://discord.com/api/v8/applications/${this.user.id}/commands`, datapost, {
        headers: {
          Authorization: this.token
        }

      }).catch((er) => console.trace(er.data))
    }
  }
  /**
  ...
  addCommands (...commands) {
    ...
    this._addLoadables(commands, this._commandsToLoad)
    return this._loadLoadables('commands', this._commandsToLoad)
  }
