...
class DataClient extends Client {
  ...
  async setupCommands () {
    ...
    let allCommands = await axios.get(`https://discord.com/api/v8/applications/${this.user.id}/commands`, {
      headers: {
        Authorization: this.token
      }
    })
    allCommands = allCommands.data
    console.log('Fetched', allCommands)
    const cmdNames = allCommands.map((x) => x.name)

    const iter = Array.from(this.commands.keys()).filter((x) => !cmdNames.includes(x))
    for (let i = 0; i < iter.length; i++) {
      const item = iter[i]
      const ind = i
      const cmd = this.commands.get(item)
      const reqParams = cmd.parameters.map((x) => {
        return {
          name: x,
          description: x,
          type: 3,
      ...
      const optionalParams = cmd.optionalParameters.map((x) => {
        return {
          name: x,
          description: x,
          type: 3,
      ...
      const subCommands = await Promise.all(Array.from(cmd.subCommands.keys())
        .map(async (key, index) => {
          const scmd = cmd.subCommands.get(key)
          const scmdreqParams = scmd.parameters.map((x) => {
            return {
              name: x,
              description: x,
              type: 3,
          ...
          const scmdoptionalParams = scmd.optionalParameters.map((x) => {
            return {
              name: x,
              description: x,
              type: 3,
          ...
          return {
            ...
            type: 1,
            options: scmdreqParams.concat(scmdoptionalParams).length?scmdreqParams.concat(scmdoptionalParams):undefined
      ...
      console.log({
        ...
        options: subCommands.concat(reqParams).concat(optionalParams).length?subCommands.concat(reqParams).concat(optionalParams):undefined
      })
      await axios.post(`https://discord.com/api/v8/applications/${this.user.id}/commands`, {
        name: item,
        description: cmd.description,
        options: subCommands.concat(reqParams).concat(optionalParams)
      }, {
        headers: {
          Authorization: this.token
        }
      ...
      console.log(`Uploaded, or tried ${item}`)
    }
  }
  /**
  ...
  _onReady () {
    ...
    this._setOwner()
    this.sm.initialize()
    this.setupCommands().catch((er) => console.trace(er))
  }
