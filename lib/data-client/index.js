...
class DataClient extends Client {
  ...
  async setupCommands () {
    ...
    const allCommands = await axios.get(`https://discord.com/api/v8/applications/${this.user.id}/commands`, {
      headers: {
    ...
    Array.from(this.commands.keys()).filter((x) => !cmdNames.includes(x))
      .forEach(async (item, ind) => {
